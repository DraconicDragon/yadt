...
class _db:
    ...
    def get_recent_datasets(self) -> list[str]:
        with self._conn() as conn:
            ...
            rows = cursor.execute(
                "select max(s.dataset) from dataset_stats s left join dataset_history h on s.id = h.dataset_id group by s.id order by max(h.id) desc, s.id desc limit 10"
            ).fetchall()
    ...
    def update_recent_datasets(self, last_dataset: str):
        ...
        with self._conn() as conn:
            ...
            try:
                try:
                    rows = cursor.execute(
                        "insert or abort into dataset_stats (dataset) values (?) returning id", (last_dataset,)
                    ).fetchall()
                    conn.commit()
                ...
                cursor.execute(
                    "delete from dataset_history where id not in (select id from dataset_history order by id desc limit 10)"
                )
    ...
    def get_dataset_setting(self, dataset: str, key: str, default=None):
        with self._conn() as conn:
            ...
            rows = cursor.execute(
                "select value from dataset_settings where dataset = ? and key = ?", (dataset, key)
            ).fetchall()
            if len(rows) == 0:
                return default
    ...
    def set_dataset_setting(self, dataset: str, key: str, value: str):
        with self._conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "insert or replace into dataset_settings (dataset, key, value) values (?, ?, ?)", (dataset, key, value)
            )
    ...
    def get_dataset_cache(self, hash: bytes, repo_name: str):
        with self._conn() as conn:
            ...
            rows = cursor.execute(
                "select c.data from dataset_cache c inner join dataset_file_hash h on h.id = c.hash_id where h.hash = ? and c.repo_name = ? limit 1",
                (hash, repo_name),
            ).fetchall()
            if len(rows) == 0:
                return None
    ...
    def get_dataset_cache_usage_for_repo_name(self):
        with self._conn() as conn:
            cursor = conn.cursor()
            rows = cursor.execute(
                "select sum(length(data)), repo_name from dataset_cache group by repo_name"
            ).fetchall()

            return [{"repo_name": row[1], "bytes": row[0]} for row in rows]
    ...
    def get_dataset_cache_for_dataset(self):
        with self._conn() as conn:
            ...
            rows = cursor.execute(
                "select d.dataset from dataset_cache c left join dataset_cache_stats s on c.id = s.hash_id left join dataset_stats d on d.id = s.dataset_id group by d.dataset"
            ).fetchall()
    ...
    def get_dataset_cache_usage_for_dataset(self):
        with self._conn() as conn:
            ...
            rows = cursor.execute(
                "select sum(length(c.data)), d.dataset from dataset_cache c left join dataset_cache_stats s on c.id = s.hash_id left join dataset_stats d on d.id = s.dataset_id group by d.dataset"
            ).fetchall()

            return [{"dataset": row[1], "bytes": row[0]} for row in rows]
    ...
    def set_dataset_cache(self, hash: bytes, repo_name: str, dataset: str, data: bytes):
        ...
        with self._conn() as conn:
            ...
            try:
                try:
                    rows = cursor.execute(
                        "insert or abort into dataset_stats (dataset) values (?) returning id", (dataset,)
                    ).fetchall()
                    conn.commit()
                except sqlite3.IntegrityError:
                ...
                try:
                    rows = cursor.execute(
                        "insert or abort into dataset_file_hash (hash) values (?) returning id", (hash,)
                    ).fetchall()
                    conn.commit()
                except sqlite3.IntegrityError:
                ...
                try:
                    cursor.execute(
                        "insert or abort into dataset_cache (hash_id, repo_name, data) values (?, ?, ?)",
                        (hash_id, repo_name, data),
                    )
                    conn.commit()
                except sqlite3.IntegrityError:
                    pass
                ...
                try:
                    cursor.execute(
                        "insert or abort into dataset_cache_stats (dataset_id, hash_id) values (?, ?)",
                        (dataset_id, hash_id),
                    )
                    conn.commit()
                except sqlite3.IntegrityError:
    ...
    def get_dataset_edit(self, dataset: str, hash: bytes):
        with self._conn() as conn:
            ...
            rows = cursor.execute(
                "select e.previous_edit, e.new_edit from dataset_manual_edit e left join dataset_stats s on s.id = e.dataset_id left join dataset_file_hash h on h.id = e.hash_id where s.dataset = ? and h.hash = ? limit 1",
                (dataset, hash),
            ).fetchall()
            if len(rows) == 0:
                return None
    ...
    def set_dataset_edit(self, dataset: str, hash: bytes, previous_edit: str, new_edit: str):
        ...
        with self._conn() as conn:
            ...
            try:
                ...
                cursor.execute(
                    "insert or replace into dataset_manual_edit (dataset_id, hash_id, previous_edit, new_edit) values (?, ?, ?, ?)",
                    (dataset_id, hash_id, previous_edit, new_edit),
                )
                conn.commit()
            except sqlite3.Error as e:
                conn.rollback()
