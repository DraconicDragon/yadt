...
class InitialOnlyImageTagger(nn.Module):
    ...
    def __init__(self, total_tags, dataset, model_name="efficientnet_v2_l", dropout=0.1, pretrained=True):
        super().__init__()
    ...
    def preprocess_image(self, img, image_size=512):
    ...
    def predict(self, image_path, threshold=0, category_thresholds=None):
        """
    ...
    def get_tags_from_predictions(self, predictions, probabilities=None):
        """
...
# class FlashAttentionCPU(nn.MultiheadAttention):
...
#         return super().forward(query, key, value, attn_mask=mask)[0]


class FlashAttention(nn.Module):
    def __init__(self, dim, num_heads=8, dropout=0.1, batch_first=True, cpu=False):
        super().__init__()
        self.dim = dim
    ...
    def forward(
        ...
        is_cpu = next(self.parameters()).is_cpu
        ...
        if mask is not None:
            ...
            if mask.size(-1) != seq_len:
                ...
                new_mask = torch.zeros(batch_size, 1, seq_len, seq_len, device=mask.device, dtype=mask.dtype)
...
class FlashAttentionCPU(FlashAttention):
    def __init__(self, dim, num_heads=8, dropout=0.1, batch_first=True):
        super().__init__(dim, num_heads, dropout, batch_first=batch_first, cpu=True)
...
class OptimizedTagEmbedding(nn.Module):
    def __init__(self, num_tags, embedding_dim, num_heads=8, dropout=0.1):
        ...
        self.attention = FlashAttentionCPU(embedding_dim, num_heads, dropout)
...
class ImageTagger(nn.Module):
    def __init__(
        ...
        self.tag_attention = FlashAttentionCPU(self.embedding_dim, num_heads, dropout)
        ...
        self.cross_attention = FlashAttentionCPU(self.embedding_dim, num_heads, dropout)
    ...
    def _get_selected_tags(self, logits):
        ...
        topk_values, topk_indices = torch.topk(probs, k=self.tag_context_size, dim=1, largest=True, sorted=True)
    ...
    def preprocess_image(self, img: Image, image_size=512):
        """Process an image for inference using same preprocessing as training"""
        ...
        try:
            ...
            raise Exception(f"Error processing {image_path}: {str(e)}") from e
...
def load_model(model_dir, full=False, device="cpu", **kwargs):
    """Load model with better error handling and warnings"""
    # print(f"Loading model from {model_dir}")

    metadata_path = kwargs.pop("metadata_path", None)
    model_info_path = kwargs.pop("model_info_path", None)
    state_dict_path = kwargs.pop("state_dict_path", None)

    # Load metadata
    metadata_path = metadata_path or os.path.join(model_dir, "metadata.json")
    ...
    # Load model info
    model_info_path = model_info_path or os.path.join(model_dir, "model_info.json")
    if os.path.exists(model_info_path):
        with open(model_info_path, "r") as f:
            model_info = json.load(f)
    else:
        print("WARNING: Model info file not found, using default settings")
        model_info = {"tag_context_size": 256, "num_heads": 16, "precision": "float16"}
    ...
    if full:
        ...
        model = InitialOnlyImageTagger(total_tags=metadata["total_tags"], dataset=dataset, pretrained=False)

    # Load weights
    state_dict_path = state_dict_path or os.path.join(
        model_dir, "model_refined.pt" if full else "model_initial_only.pt"
    )
    ...
    model = model.float()
    # print("✓ Model converted to full precision")
    ...
    if os.path.exists(thresholds_path):
        ...
        # print(f"✓ Loaded thresholds.json")

    return model, dataset, thresholds
...
if __name__ == "__main__":
    ...
    for full in (False, True):
        # Load model
        model, dataset, thresholds = load_model(model_dir, full=full)
        ...
        if len(sys.argv) > 2:
            ...
            if thresholds and "categories" in thresholds:
                print("Using category thresholds.")
                category_thresholds = {
                    cat: opt["balanced"]["threshold"] for cat, opt in thresholds["categories"].items()
                }
            ...
            # Get tags
            tags = model.get_tags_from_predictions(
