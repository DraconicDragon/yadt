...
def dataset_cache_usage_for_repo_name():
    return sorted(
        [
            [row["repo_name"], ui_utils.human_readable_bytes(row["bytes"])]
            for row in db.get_dataset_cache_usage_for_repo_name()
        ],
...
def dataset_cache_usage_for_dataset():
    return sorted(
        [
            [row["dataset"] or "UNKNOWN", ui_utils.human_readable_bytes(row["bytes"])]
            for row in db.get_dataset_cache_usage_for_dataset()
        ],
        key=lambda r: ui_utils.natural_sort(r[0]),
    )
...
def ui(args):
    with gr.Row():
        with gr.Column():
            ...
            with gr.Column(variant="panel"):
                with gr.Row(equal_height=True):
                    dataset_cache_for_repo_name_dropdown = gr.Dropdown(
                        choices=dataset_cache_for_repo_name(),
                        value=ui_utils.NO_DROPDOWN_SELECTION,
                        interactive=True,
                ...
                with gr.Row(equal_height=True):
                    dataset_cache_for_dataset_dropdown = gr.Dropdown(
                        choices=dataset_cache_for_dataset(),
                        value=ui_utils.NO_DROPDOWN_SELECTION,
                        interactive=True,
            ...
            gr.HTML(
                '<p style="margin-top: -1em"><i>WARNING: resetting the database will drop all caches and settings</i></p>'
            )
        ...
        with gr.Column():
            ...
            with gr.Column(variant="panel"):
                gr.HTML(
                    '<p>Cache usage by dataset</p><p style="font-size: 0.8em"><i>Caches might be shared between datasets, so total cache usage might be lower</i></p>'
                )
